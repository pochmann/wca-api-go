...
for path in sorted(glob(join('data', 'WCA_export_*.tsv'))):
    ...
    with open(path) as tf:
        ...
        types.append((typename, varname, fieldnames, fieldtypes, path))
...
with open('wca-data.go', 'w') as f:
    ...
    p('// This code was generated by generate_code.py and should not be edited.')
    p('package main')
    p('import ("bufio"\n"os"\n"strings")')

    # Output the variable declarations
    for typename, varname, fieldnames, fieldtypes, path in types:
	...
	for i := range {varname} {{
		scanner.Scan()
		row := strings.Split(scanner.Text(), "\\t")'''.format(path=path, varname=varname, typename=typename))
